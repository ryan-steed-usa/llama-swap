...
LS_REPO="ryan-steed-usa/llama-swap"
...
for CONTAINER_TYPE in non-root root; do
  ...
  if [ "$CONTAINER_TYPE" == "non-root" ]; then
    CONTAINER_TAG="${CONTAINER_TAG/llama-swap:/llama-swap-non-root:}"
    CONTAINER_LATEST="${CONTAINER_LATEST/llama-swap:/llama-swap-non-root:}"
    USER_UID=10001
    USER_GID=10001
    USER_HOME=/app
  fi
