...
LS_VER=$(curl -s https://api.github.com/repos/${GITHUB_REPOSITORY:-mostlygeek/llama-swap}/releases/latest | jq -r .tag_name | sed 's/v//')
...
CONTAINER_TAG="ghcr.io/${GITHUB_REPOSITORY:-mostlygeek/llama-swap}:v${LS_VER}-${ARCH}-${LCPP_TAG}"
CONTAINER_LATEST="ghcr.io/${GITHUB_REPOSITORY:-mostlygeek/llama-swap}:${ARCH}"
